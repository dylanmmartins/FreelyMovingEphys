"""Head-fixed checkerboard stimulus.
"""
import cv2
import numpy as np
...
def calc_flash_PSTH(rpath):
    ...
    stim_path = fmUtil.path.find()
    stim_data = fmUtil.file.read_h5(stim_path)

    vid = stim_data['world_video'].astype(np.uint8)
    stimT = stim_data['timestamps']

    spikeT = stim_data['spikeT']
    ...
    for i, sps in tqdm(spikeT.items()):
        ...
        all_psth[i,:] = fmUtil.ephys.calc_PSTH(sps, eventT)
...
def calc_laminar_depth(cfg, rpath):

    ephysbin_path = fmUtil.path.find()

    probe_name = cfg['probe']
    nCh, spacing = fmUtil.ephys.probe_to_ch(probe_name)

    ephys = fmUtil.ephys.read_ephysbin(ephysbin_path, nCh, probe_name)

    # Mean-center and bandpass filter
    ephys = ephys - np.mean(ephys, 0)
    ephys = fmUtil.filter.butter_filt(ephys, order=6)
